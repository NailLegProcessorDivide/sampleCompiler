...
fn init_var_dec_to_0(dec: &VarDec) -> Stmt {
    Stmt::Assign(dec.var_name.clone(), Vec::new(), TypedExp {exp: Exp::Num(0), typ: Some(Typ::Int)})
}
...
pub fn build_ast(func: &Func) -> Vec<Stmt> {
    func.locals.iter().map(|local: &VarDec| init_var_dec_to_0(&local)).chain(
...
pub fn build_annot_cfg(globals: &HashSet<Var>, func: &Func) -> Vec<(CFGEntry, CFGAnnot)> {
    ...
    live_var_analysis::lva(globals, &cfg).iter().map(|(e, a)| ((*e).clone(), a.clone())).collect()
}
