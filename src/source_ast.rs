...
fn parse_typ (toks: &[TokLoc]) -> Result<(Typ, &[TokLoc]), String> {
    match toks {
        [TokLoc {tok: Token::Int, loc: _}, toks @ ..] => Ok((Typ::Int, toks)),
        [TokLoc {tok: Token::Bool, loc: _}, toks @ ..]=> Ok((Typ::Bool, toks)),
        [TokLoc {tok: Token::Array, loc: _}, TokLoc {tok: Token::Num(array_size), loc: _}, toks @ ..] => Ok((Typ::Array(*array_size as usize), toks)),
...
fn parse_args(mut toks: &[TokLoc]) -> Result<(Vec<TypedExp>, &[TokLoc]), String> {
    ...
    loop {
        ...
        match toks {
            ...
            _ => return Err(format!("expected ',' or ')' found {}", show_token(&(toks.get(0).unwrap()).tok)))
        }
    }
}
        ...
        match toks {
            ...
            [TokLoc{tok: Token::Lbrac, loc: _}, rem_toks @ ..] => {
                match parse_exp(rem_toks)? {
                    ...
                    _ => return Err(format!("unexpected token {} while parsing indicies", show_token(&(rem_toks.get(0).unwrap()).tok)))
                }
            },
...
fn parse_atomic_exp(toks: &[TokLoc]) -> Result<(TypedExp, &[TokLoc]), String> {
    match toks {
        ...
        [TokLoc{tok: Token::Ident(ident), loc: _}, TokLoc{tok: Token::Lparen, loc: _}, toks @ ..] => {
            ...
            Ok((TypedExp {exp: Exp::Call(ID::Source(ident.clone(), None), args), typ: None}, toks))
        },
        [TokLoc{tok: Token::Ident(ident), loc: _}, toks @ ..] => {
            let (indicies, toks) = parse_indices(toks)?;
            Ok((TypedExp {exp: Exp::Ident(ID::Source(ident.to_string(), None), indicies), typ: None}, toks))
        },
        [TokLoc{tok: Token::Num(n), loc: _}, toks @ ..] => Ok((TypedExp {exp: Exp::Num(*n), typ: None}, toks)),
        [TokLoc{tok: Token::True, loc: _}, toks @ ..] => Ok((TypedExp {exp: Exp::Bool(true), typ: None}, toks)),
        [TokLoc{tok: Token::False, loc: _}, toks @ ..] => Ok((TypedExp {exp: Exp::Bool(false), typ: None}, toks)),
        [TokLoc{tok: Token::Op(OP::Minus), loc: _}, toks @ ..] => {
            let (exp, toks) = parse_atomic_exp(toks)?;
            Ok((TypedExp {exp: Exp::Op(Box::new(TypedExp {exp: Exp::Num(0), typ: None}), OP::Minus, Box::new(exp)), typ: None}, toks))
        },
        [TokLoc{tok: Token::Uop(uop), loc: _}, toks @ ..] => {
            let (exp, toks) = parse_atomic_exp(toks)?;
            Ok((TypedExp {exp: Exp::Uop(uop.clone(), Box::new(exp)), typ: None}, toks))
        },
        [TokLoc{tok: Token::Array, loc: _}, toks @ ..] => {
            let (inds, toks) = parse_indices(toks)?;
            Ok((TypedExp {exp: Exp::Array(inds), typ: None}, toks))
        },
        [TokLoc{tok: Token::Lparen, loc: _}, toks @ ..] => {
            match parse_exp(toks)? {
                (exp, [TokLoc{tok:Token::Rparen, loc: _}, toks @ ..]) => Ok((exp, toks)),
                (_, []) => return Err("eof parsing bracketed expression".to_string()),
                _ => return Err(format!("expected ')' found {}", show_token(&(toks.get(0).unwrap()).tok)))
            }
        }
        _ => return Err(format!("unimplemented exp starting with '{}' within '{}'", show_token(&(toks.get(0).unwrap()).tok), &(toks.get(0).unwrap()).loc))
    }
...
fn parse_exp(toks: &[TokLoc]) -> Result<(TypedExp, &[TokLoc]), String> {
    match parse_atomic_exp(toks)? {
        (e1, [TokLoc{tok: Token::Op(o), loc: _}, toks @ ..]) => {
            ...
            Ok((TypedExp {exp: Exp::Op(Box::new(e1), o.clone(), Box::new(e2)), typ: None}, toks))
        },
         ...
         TokLoc {tok: Token::Colon, loc: _}, rem_toks @ ..] => {
            ...
            match (typ, toks) {
                (t, [TokLoc{tok: Token::Op(OP::Eq), loc: _}, rem_toks @ ..]) => {
                    ...
                    let (exp, rem_toks) = parse_exp(toks)?;
                    toks = rem_toks;
                    Ok((VarDec { var_name: ID::Source(x.to_string(), None), typ: t, init: exp, loc: Some(*line_number)}, toks))
                },
                _ => return Err(format!("invalid vardec line {}", line_number))
            }
        }
        _ => return Err(format!("expected variable declaration {}: '{}'", toks.get(0).unwrap().loc, show_token(&(toks.get(0).unwrap()).tok)))
    }
}
        ...
        [TokLoc{tok: Token::Lparen, loc: _},
         ...
         toks @ ..] => {
            match parse_typ(toks)? {
                ...
                (_, [TokLoc{tok: _, loc}, ..]) => Err(format!("error parsing function parameter on line {}", *loc)),
            }
        },
        [TokLoc{tok, loc}, ..] => Err(format!("error parsing function parameter on line {} starting '{}'", *loc, show_token(tok)))
    }
}
...
fn parse_stmt(toks: &[TokLoc]) -> Result<(Stmt, &[TokLoc]), String> {
    match toks {
        ...
        [TokLoc{tok: Token::Ident(x), loc: ln}, toks @ ..] => {
            match parse_indices(toks)? {
                ...
                (indices, [TokLoc{tok: Token::Assign, loc: _}, toks @ ..]) => {
                    ...
                    Ok((Stmt::Loc(Box::new(Stmt::Assign(ID::Source(x.to_string(), None), indices, exp)), *ln), toks))
                },
                (_, [TokLoc{tok, loc}, ..]) => Err(format!("error parsing function parameter on line {} starting '{}'", *loc, show_token(tok))),
            }
        },
        [TokLoc{tok: Token::While, loc: ln}, toks @ ..] => {
            let (e, toks) = parse_exp(toks)?;
            let (s, toks) = parse_stmt(toks)?;
            Ok((Stmt::Loc(Box::new(Stmt::DoWhile(Box::new(Stmt::Stmts(Vec::new())), e, Box::new(s))), *ln), toks))
        },
        [TokLoc{tok: Token::Do, loc: _}, toks @ ..] => {
            match parse_stmt(toks)? {
                ...
                (s, [TokLoc{tok: Token::While, loc: ln}, toks @ ..]) => {
                    let (e, toks) = parse_exp(toks)?;
                    Ok((Stmt::Loc(Box::new(Stmt::DoWhile(Box::new(s), e, Box::new(Stmt::Stmts(Vec::new())))), *ln), toks))
                }
                (_, [TokLoc{tok, loc}, ..]) => Err(format!("error parsing do statement on line {} starting '{}'", *loc, show_token(tok)))
            }
        },
                ...
                (cond, [TokLoc{tok: Token::Then, loc: ln}, toks @ ..]) => {
                    match parse_stmt(toks)? {
                        ...
                        (then_exp, [TokLoc{tok: Token::Else, loc: ln}, toks @ ..]) => {
                            ...
                            Ok((Stmt::Loc(Box::new(Stmt::Ite(cond, Box::new(then_exp), Box::new(else_exp))), *ln), toks))
                        },
                        (_, [TokLoc{tok, loc}, ..]) => Err(format!("error parsing else on line {} starting '{}'", *loc, show_token(tok))),
                    }
                },
                (_, [TokLoc{tok, loc}, ..]) => Err(format!("error parsing then parameter on line {} starting '{}'", *loc, show_token(tok))),
            }
        },
        ...
        [TokLoc{tok: Token::Input, loc: ln},
         TokLoc{tok: Token::Ident(x), loc: _},
         toks @ ..] => 
             Ok((Stmt::Loc(Box::new(Stmt::In(ID::Source(x.to_string(), None))), *ln), toks)),
        [TokLoc{tok: Token::Output, loc: ln},
         TokLoc{tok: Token::Ident(x), loc: _},
         toks @ ..] =>
             Ok((Stmt::Loc(Box::new(Stmt::Out(ID::Source(x.to_string(), None))), *ln), toks)),
        [TokLoc{tok: Token::Return, loc: ln},
         TokLoc{tok: Token::Ident(x), loc: _},
         toks @ ..] =>
             Ok((Stmt::Loc(Box::new(Stmt::Return(Some(ID::Source(x.to_string(), None)))), *ln), toks)),
        [TokLoc{tok, loc}, ..] => Err(format!("error parsing statement on line {} starting '{}'", *loc, show_token(tok)))
    }
}
...
fn parse_stmt_list(mut toks: &[TokLoc]) -> Result<(Vec<Stmt>, &[TokLoc]), String> {
    ...
    loop {
        match toks {
            [TokLoc{tok: Token::Rcurly, loc: _}, rem_toks @ ..] => return Ok((stmts, rem_toks)),
            _ => {
    ...
    match toks {
        ...
        [TokLoc{tok: Token::Function, loc: func_loc}, TokLoc{tok: Token::Ident(func_name), loc: _}, toks @ ..] => {
            match parse_param_list(toks)? {
                    ...
                    if params.len() != 0 {
                        match parse_typ(toks)? {
                            ...
                            (t, [TokLoc{tok: Token::Lcurly, loc: _}, toks @ ..]) => {
                                ...
                                return Ok((Func{ fun_name: ID::Source(func_name.to_string(), None), params: params, ret: t, locals: var_decs, body: stmts, loc: Some(*func_loc) }, toks))
                            },
                            (_,  [TokLoc{tok: _, loc}, ..]) => return Err(format!("error parsing function expected '{{' on line {}", *loc)),
                        }
                    }
                    else {
                ...
                (_, [TokLoc{tok: _, loc}, ..]) => return Err(format!("error parsing function on line {}", *loc)),
            }
        },
        [TokLoc{tok: _, loc}, ..] => return Err(format!("error parsing function on line {}", *loc))
    }
}
...
pub fn parse_program(toks: &[TokLoc]) -> Result<Prog, String> {
    ...
    Ok(Prog { globals: globals, funcs: funcs })
}
