use crate::{
    source_ast::{Exp, Func, Prog, Scope, Stmt, Typ, TypedExp, VarDec, ID},
    tokens::{OP, UOP},
};
...
fn add_scope(id: &mut ID, scope: Scope) {
    match id {
        ID::Source(name, None) => {
            *id = ID::Source(name.clone(), Some(scope));
        }
        ID::Source(_, Some(_)) => {}
        ID::Temp(name, _) => {}
    }
...
fn type_exp(ln: Option<usize>, env: &Env, exp: &mut TypedExp) -> Result<Typ, String> {
    let typ: Typ = match &mut exp.exp {
        Exp::Ident(id, es) => type_ident(ln, env, id, es)?,
        Exp::Call(f, args) => {
        ...
        Exp::Op(e1, op, e2) => {
            ...
            match (t1, op, t2) {
                ...
                (_, _, _) => {
                    return Err(format!("no opetator matching op on line: {}", ln.unwrap()))
                }
            }
        }
        Exp::Uop(uop, exp) => {
...
fn type_var_dec(sc: &Scope, env: &Env, dec: &mut VarDec) -> Option<String> {
    ...
    if typ != dec.typ {
        match dec.var_name.clone() {
            ID::Source(name, _) => Some(format!(
                "type mismatch var '{}' on line '{}'",
                name,
                dec.loc.unwrap()
            )),
            ID::Temp(name, _) => Some(format!(
                "type mismatch var '{}' on line '{}'",
                name,
                dec.loc.unwrap()
            )),
...
fn get_function_types(funcs: &[Func]) -> Result<HashMap<ID, (Vec<Typ>, Typ)>, String> {
    ...
    for func in funcs.iter() {
        if func_types.contains_key(&func.fun_name) {
            ...
            func_types.insert(
                func.fun_name.clone(),
                (func.params.iter().map(|(_, typ)| *typ).collect(), func.ret),
            );
        }
    }
    Ok(func_types)
...
fn type_stmt(ln: Option<usize>, env: &Env, ret: Typ, stmt: &mut Stmt) -> Option<String> {
    match stmt {
        ...
        Stmt::Return(_) => return Some(format!("wut")),
        Stmt::Assign(x, es, e) => {
...
pub fn type_prog(prog: &mut Prog) -> Option<String> {
    let env = Env {
        funcs: match get_function_types(&prog.funcs) {
            Ok(v) => v,
            Err(e) => return Some(e),
        },
