...
use crate::{
    source_ast::{scope_to_string, Exp, Scope, Stmt, TypedExp, ID},
    tokens::{OP, UOP},
};
...
pub fn test_to_string(test: &Test) -> String {
    format!(
        "{} {} {}",
        atomic_to_string(&test.exp1),
        test_op_to_str(test.op.clone()),
        atomic_to_string(&test.exp2)
    )
}
...
fn flat_e_to_assign(x: &ID, e: &TypedExp) -> Vec<BlockElem> {
    ...
    match &e.exp {
        ...
        Exp::Op(ae1, op, ae2) => vec![BlockElem::AssignOp(
            v,
            exp_to_atomic(ae1),
            *op,
            exp_to_atomic(ae2),
        )],
        Exp::Uop(UOP::Not, ae) => vec![BlockElem::AssignOp(
            v,
            exp_to_atomic(ae),
            OP::Eq,
            AtomicExp::Num(0),
        )],
        Exp::Array(es) => vec![BlockElem::Call(
            Some(v),
            format!("allocate{}", es.len()),
            es.iter().map(|e| exp_to_atomic(e)).collect(),
        )],
        Exp::Call(f, es) => {
            let name = match f {
                ID::Source(s, _) => s,
...
fn flat_exp_to_test(e: &Exp) -> Test {
    match e {
        Exp::Ident(id, inds) if inds.len() == 0 => Test {
            exp1: AtomicExp::Ident(id_to_var(id)),
            op: TestOp::Eq,
            exp2: AtomicExp::Num(1),
        },
        Exp::Call(_, _) => panic!("call in test"),
        Exp::Num(_) => panic!("num in test"),
        Exp::Bool(b) => Test {
            exp1: AtomicExp::Num((*b) as i64),
            op: TestOp::Eq,
            exp2: AtomicExp::Num(1),
        },
        Exp::Op(id1, op, id2) => Test {
            exp1: exp_to_atomic(&**id1),
            op: atomic_op_to_test_op(op),
            exp2: exp_to_atomic(&**id2),
        },
        Exp::Uop(UOP::Not, id) => Test {
            exp1: exp_to_atomic(&**id),
            op: TestOp::Eq,
            exp2: AtomicExp::Num(0),
        },
...
fn build_cfg(
    ...
    for stmt in ast {
        match stmt {
            ...
            Stmt::Stmts(ss) => {
                blocks.push(make_cfg_entry(
                    wip_bnum,
                    wip_elems,
                    NextBlock::Next(*block_num),
                ));
                ...
                *block_num += 1;
                let mut header = if let Stmt::Stmts(stmts0) = &**s0 {
                    build_cfg(
                        ...
                        block_num,
                        s0_b,
                        &NextBlock::Branch(flat_exp_to_test(&e.exp), s1_b, wip_bnum),
                    )
                } else {
                    build_cfg(
                        &(vec![*s0.clone()]),
            ...
            Stmt::Ite(e, s0, s1) => {
                ...
                *block_num += 1;
                blocks.push(make_cfg_entry(
                    wip_bnum,
                    wip_elems,
                    NextBlock::Branch(flat_exp_to_test(&e.exp), s0_b, s1_b),
                ));
                wip_bnum = *block_num;
                *block_num += 1;
            ...
            Stmt::Out(x) => {
                wip_elems.push(BlockElem::Call(
                    None,
                    String::from("output"),
                    vec![AtomicExp::Ident(id_to_var(&x))],
                ));
            }
            Stmt::Loc(s0, _) => {
                let s0_b = *block_num;
            ...
            Stmt::Return(Some(id)) => {
                blocks.push(make_cfg_entry(
                    wip_bnum,
                    wip_elems,
                    NextBlock::Return(Some(id_to_var(&id))),
                ));
                wip_bnum = *block_num;
                *block_num += 1;
                wip_elems = Vec::new();
...
fn blk_to_cheap(blk: &CFGEntry) -> CheapBlk {
    CheapBlk {
        bnum: blk.bnum,
        elems: blk.elems.len(),
        next: blk.next.clone(),
...
//crap code to make the cfg less terrible
pub fn clean_cfg(cfg: &mut Vec<CFGEntry>) {
    let mut cheap_list = cfg.iter().map(|b| blk_to_cheap(b)).collect::<Vec<_>>();
    while {
        let mut del_list = HashSet::new();
        let mut updated = false;
        for blk in cheap_list.iter() {
            if blk.elems == 0 {
                if let NextBlock::Next(x) = blk.next {
                    del_list.insert(blk.bnum);
                    for o_blk in cfg.iter_mut() {
                        o_blk.next = match &o_blk.next {
                            NextBlock::Return(a) => NextBlock::Return(a.clone()),
                            NextBlock::Next(n) if *n == blk.bnum => NextBlock::Next(x),
                            NextBlock::Next(n) => NextBlock::Next(*n),
                            NextBlock::Branch(_, n1, n2) if *n1 == blk.bnum && *n2 == blk.bnum => {
                                panic!("condional branch with both branches to same block")
                            }
                            NextBlock::Branch(e, n1, n2) if *n1 == blk.bnum => {
                                NextBlock::Branch(e.clone(), x, *n2)
                            }
                            NextBlock::Branch(e, n1, n2) if *n2 == blk.bnum => {
                                NextBlock::Branch(e.clone(), *n1, x)
                            }
                            NextBlock::Branch(e, n1, n2) => NextBlock::Branch(e.clone(), *n1, *n2),
                        };
                    }
                }
                for o_blk in cfg.iter_mut() {
                    match &o_blk.next {
                        NextBlock::Next(n) if *n == blk.bnum => {
                            o_blk.next = blk.next.clone();
                            updated = true
                        }
                        _ => {}
                    }
        ...
        cheap_list.retain(|blk| !del_list.contains(&blk.bnum));
        cfg.retain(|blk| !del_list.contains(&blk.bnum));
        !del_list.is_empty() || updated
    } {}

    let mut cfg_inds = vec![0 as usize];
    let mut ind_idx = 0;
    while ind_idx < cfg_inds.len() {
        match &cfg[cfg_inds[ind_idx]].next {
            NextBlock::Return(_) => {}
            NextBlock::Next(n) => {
                if !cfg_inds.contains(n) {
                    cfg_inds.push(*n)
                }
            }
            NextBlock::Branch(_, n1, n2) => {
                if !cfg_inds.contains(n1) {
                    cfg_inds.push(*n1)
                }
                if !cfg_inds.contains(n2) {
                    cfg_inds.push(*n2)
                }
            }
        }
    ...
    cfg.retain(|blk| cfg_inds.contains(&blk.bnum));
}
