use crate::source_ast::{stmts_to_stmt, Exp, Stmt, Typ, TypedExp, ID};
use crate::tokens::OP;
...
fn unnest_indices(
    arr: &ID,
    indicies: &Vec<TypedExp>,
    next_ident: &mut usize,
) -> (Vec<Stmt>, TypedExp) {
    if indicies.len() < 2 {
        (
            Vec::new(),
            TypedExp {
                exp: Exp::Ident(arr.clone(), indicies.clone()),
                typ: Some(Typ::Int),
            },
        )
    } else {
        ...
        for ex in &indicies[..indicies.len() - 1] {
            ...
            *next_ident += 1;
            res_vec.push(Stmt::Assign(
                id.clone(),
                Vec::new(),
                TypedExp {
                    exp: Exp::Ident(pid, vec![ex.clone()]),
                    typ: Some(Typ::Array(1)),
                },
            ));
            pid = id;
        }
        (
            res_vec,
            TypedExp {
                exp: Exp::Ident(pid, vec![indicies.last().unwrap().clone()]),
                typ: Some(Typ::Int),
            },
        )
    }
...
fn unnest_exp(e: &TypedExp, next_ident: &mut usize) -> (Vec<Stmt>, TypedExp) {
    match &e.exp {
        ...
        Exp::Ident(i, es) => {
            let (s_list, aes): (Vec<Vec<Stmt>>, Vec<TypedExp>) = es
                .into_iter()
                .map(|x| unnest_exp_atomic(x, next_ident))
                .unzip();
        ...
        Exp::Op(e1, op, e2) => {
            ...
            (
                ...
                TypedExp {
                    exp: Exp::Op(Box::new(a1), *op, Box::new(a2)),
                    typ: e.typ,
                },
            )
        }
        ...
        Exp::Array(es) => {
            let (s_list, aes): (Vec<Vec<Stmt>>, Vec<TypedExp>) = es
                .into_iter()
                .map(|part| unnest_exp_atomic(part, next_ident))
                .unzip();
            (
                s_list.into_iter().flatten().collect(),
                TypedExp {
                    exp: Exp::Array(aes),
                    typ: e.typ,
                },
    ...
    match s {
        Stmt::Assign(x, es, e) => {
            if es.is_empty() {
                ...
                let (s_list, aes): (Vec<Vec<Stmt>>, Vec<TypedExp>) =
                    es.iter().map(|e| unnest_exp_atomic(e, next_ident)).unzip();
        ...
        Stmt::DoWhile(s0, e, s1) => {
            ...
            vec![Stmt::DoWhile(
                Box::new(stmts_to_stmt(
                    s0s.into_iter().chain(se1.into_iter()).collect(),
                )),
                es,
                Box::new(stmts_to_stmt(s1s)),
            )]
        }
        Stmt::Ite(e, s1, s2) => {
            ...
            se.push(Stmt::Ite(
                e1,
                Box::new(stmts_to_stmt(s1s)),
                Box::new(stmts_to_stmt(s2s)),
            ));
            se
        }
        Stmt::Stmts(s_list) => s_list
            .into_iter()
            .map(|stmt| unnest_stmt(stmt, next_ident))
            .flatten()
            .collect(),
...
pub fn unnest(stmts: &Vec<Stmt>) -> Vec<Stmt> {
    ...
    stmts
        .iter()
        .map(|stmt| unnest_stmt(stmt, &mut next_ident))
        .flatten()
        .collect()
}
