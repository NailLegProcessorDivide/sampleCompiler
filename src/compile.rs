...
use crate::{source_ast::{Prog, Func, ID, Stmt, Typ, Scope}, compile_function, block_structure::Var, x86::Instruction_x64};
...
pub fn compile(prog : &mut Prog, target_filename : &str) {
    let mut main : Func = Func {
        ...
        ret: Typ::Int,
        locals: Vec::new(),
        body: prog.globals.iter().map(|var| Stmt::Assign(var.var_name.clone(), Vec::new(), var.init.clone())).collect(),
        loc: None,
    };
    ...
    let globals : HashSet<Var> = HashSet::from_iter(prog.globals.iter().map(|glob| id_to_var(&glob.var_name)));
    println!("made it to start of funcs");
    let functions : Vec<(ID, Vec<Instruction_x64>)> = prog.funcs.iter().map(|func| compile_function::compile_fun_x64(false, &globals, func)).collect();
