...
fn init_val(val_type: Typ) -> Val {
    match val_type {
        Typ::Array(dims) => Val::VArray(Rc::new(RefCell::new(ArrayVal{sizes : vec![0; dims], data : Vec::new()}))),
        _ => Val::VInt(0),
    }
}
...
fn interp_stmt(env : &mut InterpEnv, stmt: &Stmt) -> Option<Val> {
    match stmt {
        ...
        Stmt::DoWhile(head, e, body) => {
            match interp_stmt(env, &*head) {
                None => {
        ...
        Stmt::Return(Some(i)) => {
            Some(env.vars.get(i).unwrap().clone())
        }
...
fn interp_exp(env : &mut InterpEnv, exp: &Exp) -> Val {
    match exp {
        ...
        Exp::Array(iexps) => {
            let sizes : Vec<usize> = iexps.iter().map(|exp| 
                match interp_exp(env, exp) {
...
fn interp_var_decs(env : &mut InterpEnv, var_decs : &[VarDec]) {
    ...
    for var_dec in var_decs.iter() {
        env.vars.insert(var_dec.var_name.clone(), init_val(var_dec.typ));
    }
    for var_dec in var_decs.iter() {
