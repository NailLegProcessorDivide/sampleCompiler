...
use crate::block_structure::{CFGEntry, Var, NextBlock, BlockElem, AtomicExp, Test};
...
fn analyse_block(b : &Vec<BlockElem>) -> CFGAnnot {
    let mut out = CFGAnnot{gen: HashSet::new(), kill: HashSet::new(), live_exit: HashSet::new()};
    for e in b {
        match e {
...
fn find_preds<'a, 'b>(bnum : usize, cfg : &'a Vec<(&'b CFGEntry, CFGAnnot)>) -> (Vec<(&'b CFGEntry, CFGAnnot)>, Vec<(&'b CFGEntry, CFGAnnot)>) {
...
pub fn lva<'a>(globals : &HashSet<Var>, cfg : &'a Vec<CFGEntry>) -> Vec<(&'a CFGEntry, CFGAnnot)> {
    let init_list : Vec<(&CFGEntry, CFGAnnot)> = cfg.iter().map(|e| (e, init_live_exit(globals, analyse_block(&e.elems), e.next.clone()))).collect();
    do_one(init_list)
}
